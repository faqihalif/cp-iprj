...
const DataTable = props => {
    ...
    return (
        <div className="bg-white border rounded-lg">
            <div className="relative grid items-center grid-cols-2 gap-4 p-4">
                {
                    selectedFlatRows.length == 0 ? (
                        ...
                        <div className="flex items-center space-x-4">
                            <div>
                                <p className="text-sm font-medium text-slate-900">{`${selectedFlatRows.length} selected`}</p>
                            </div>
            ...
            {/* table */}
            <div className="overflow-auto rounded" style={{ maxHeight: windowSize.height - 400 }}>
                <table {...getTableProps()} className={`w-full relative ${props.fixed ? 'table-fixed' : 'table-auto'}`} ref={tableRef}>
                                    ...
                                    <tr {...restHeaderGroupProps} key={key}>
                                        {
                                            headerGroup.headers.map((column) => {
                                                ...
                                                return (
                                                    <th className="sticky top-0 left-0 h-[72px] px-4 py-2 text-xs font-semibold text-left uppercase bg-slate-900 border-b" key={key} {...restColumn} >
                                                        <div className={"flex justify-between w-full h-full " + (props.filterPerColumn && column.showFilter != false ? 'flex-col' : 'flex-row')}>
                    ...
                    <tbody {...getTableBodyProps()} className="divide-y">
                        {
                            page.length == 0 ? (
                                <tr className="h-24 hover:bg-slate-100">
                                    <td className="px-4 py-2 text-sm text-left bg-white text-slate-700" colSpan={columns.length}>
                                        <p className="text-sm text-center text-slate-900">No data available</p>
                                    </td>
                                </tr>
                            ) : (
                                page.map((row) => {
                                    ...
                                    return (
                                        <tr {...restRowProps} className="h-24 hover:bg-slate-100" key={key}>
                                            {
                                                row.cells.map(cell => {
                                                    const { key, ...restCellProps } = cell.getCellProps()
                                                    return (
                                                        <td {...restCellProps} className={`px-4 py-2 text-sm text-left text-slate-900 bg-white ${props.fixed ? 'truncate' : ''}`} key={key}>
                                                            {
                                                                cell.render('Cell')
                                                            }
                                                        </td>
            ...
            <div className="flex items-center justify-between p-4 border-t">
                ...
                <div className="flex items-center space-x-2">
                    <p className="text-xs text-slate-900">{`Showing ${page.length} results of ${rows.length} records`}</p>
                </div>
                ...
                <div className="flex items-center space-x-4 rounded">
                    ...
                    <div className="flex items-center">
                        <button
                            type="button"
                            className={`rounded-r-none rounded-l p-1 flex items-center justify-center ${!canPreviousPage ? 'text-slate-300' : 'text-slate-900'}`}
                            onClick={() => gotoPage(0)}
                        ...
                        <button
                            type="button"
                            className={`rounded-none p-1 flex items-center justify-center ${!canPreviousPage ? 'text-slate-300' : 'text-slate-900'}`}
                    ...
                    {/* center */}
                    <div>
                        <p className="text-xs text-slate-900">{`Page ${state.pageIndex + 1} of ${pageOptions.length == 0 ? '1' : pageOptions.length}`}</p>
                    </div>
                    ...
                    <div className="flex items-center">
                        <button
                            type="button"
                            className={`rounded-none p-1 flex items-center justify-center ${!canNextPage ? 'text-slate-300' : 'text-slate-900'}`}
                            onClick={() => nextPage()}
                        ...
                        <button
                            type="button"
                            className={`rounded-l-none rounded-r p-1 flex items-center justify-center ${!canNextPage ? 'text-slate-300' : 'text-slate-900'}`}
...
const CheckBox = React.forwardRef(({ indeterminate, ...rest }, ref) => {
    ...
    return (
        <input type="checkbox" className="w-4 h-4 mt-1 text-slate-900" ref={resolvedRef} {...rest} />
    )
})
